...
cleanup(){
  ...
  # restore stdin from fd #6, where it had been saved, and close file descriptor #6 ( 6<&- ) to free it for other processes to use ( "<&6 6<&-" is said to also works )
  exec 0<&6 6<&-
...
# hack: redirect stdin to another file descriptor, now we're not using read -s to prevent stuff from appearing
exec 6<&0
# replace stdin by /dev/null
#exec < /dev/null
