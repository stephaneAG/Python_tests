...
try:
  opts, args = getopt.getopt(sys.argv[1:], "hp:b:", ["port=", "baud="])
except getopt.GetoptError:
  print 'unsupported argument(s) !'
  print 'madtoyslab_XboxContrllerS_companion.py -p <port> -b <baudrate>'
  sys.exit(2)
for opt, arg in opts:
  if opt in ("-h", "--help"): # handle help ( handier than " opt == '-h': " )
    print 'madtoyslab_XboxContrllerS_companion.py -p <port> -b <baudrate>'
    print 'default port: %s' % (port)
    print 'default baudrate: %s' % (baud)
    sys.exit()
  elif opt in ("-p", "--port"):
    port = arg
  elif opt in ("-b", "--baud"):
    baud = arg
  # R: add handling for: debug / clocking / controls reset params / Ar counterpart
...
try:
    ...
    ser = serial.Serial(port, baud, timeout=None) # now with default params & cli args
